---
name: html-report
description: Use when 生成 HTML 格式的报告（report、网页报告、可视化总结等）。约束术语悬停注释 + 护眼视觉两条强制规则。
---

# HTML 报告规则

生成 **HTML 格式报告**时，以下两条强制规则一律遵守。

---

## 规则一：术语悬停注释（强制）

对读者可能不懂的词，一律加**悬停/点击即显的注释**，目标「小白一眼看明白，不打断阅读」。
...
用 `<span class="term">` + 自定义 tooltip，桌面悬停、手机点击都生效：
...
> 报告较长时，统一在 `<head>` 放一份上述样式，正文只写 `<span class="term">` 即可。

---

## 规则二：护眼视觉（强制）

视觉上一律白色底、护眼舒服，目标「长时间看不累、手机桌面都顺眼」。
