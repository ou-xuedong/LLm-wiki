...
**上位概念**：[[模块化|模块化]], [[极简主义|极简主义]], [[组合性|组合性]]
**相关概念**：[[MCP协议|MCP协议]], [[wedge_安全可观测性_ai_和大模型系统的安|可观测性]], [[结构化输出|结构化输出]], [[LLM抽象层|LLM抽象层]], [[技术债务|技术债务]]
**关键人物**：[[Eric-S-Raymond|Eric_S_Raymond]]
...
**现代变体**：
- [[MCP协议|MCP协议]] = Unix管道在AI Agent时代的重实现
- [[LLM抽象层|LLM抽象层]] = Unix库抽象的LLM版
- [[结构化输出|结构化输出]] = 纯文本协议的AI时代版本
...
| Unix原概念 | AI时代对应 |
|-----------|-----------|
| 管道(Pipe) | [[MCP协议|MCP协议]]、Agent间通信 |
| 过滤器(Filter) | [[ReAct模式|ReAct模式]]的单步操作 |
| 纯文本协议 | [[结构化输出|结构化输出]]（JSON/Markdown） |
| 进程(Process) | [[LLM抽象层|LLM抽象层]] |
| 环境变量 | [[Prompt模板]] |
| 透明性 | [[wedge_安全可观测性_ai_和大模型系统的安|wedge_安全可观测性_ai_和大模型系统的安]] + Chain-of-Thought |

---

**主人学习进度**：课程大纲已建立（[[unix编程艺术_ai时代高质量产品设计课程|unix编程艺术_ai时代高质量产品设计课程]]），待主人确认后开始第一讲。
